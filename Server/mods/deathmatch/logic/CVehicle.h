...
public:
    ...
    std::uint16_t GetAdjustableProperty() const noexcept { return m_usAdjustableProperty; };
    void          SetAdjustableProperty(std::uint16_t usAdjustable) noexcept {
        m_usAdjustableProperty = usAdjustable;
    }
    ...
    class CPlayer* GetSyncer() const noexcept { return m_pSyncer; };
    void           SetSyncer(CPlayer* pPlayer) noexcept;

    bool IsUnoccupiedSyncable() const noexcept { return m_bUnoccupiedSyncable; };
    void SetUnoccupiedSyncable(bool bUnoccupiedSynced) noexcept { m_bUnoccupiedSyncable = bUnoccupiedSynced; };

    std::uint8_t GetMaxPassengers();
    std::uint8_t GetFreePassengerSeat();

    void SetMaxPassengers(std::uint8_t ucPassengers) noexcept { m_ucMaxPassengersOverride = ucPassengers; };

    CVehicleUpgrades* GetUpgrades() const noexcept { return m_pUpgrades; }
    void              SetUpgrades(CVehicleUpgrades* pUpgrades) noexcept;
    ...
    std::uint32_t GetTimeSinceLastPush() { return (std::uint32_t)(CTickCount::Now(true) - m_LastPushedTime).ToLongLong(); }
...
private:
    ...
    std::uint16_t     m_usLastUnoccupiedSyncDimension;

public:            // 'Safe' variables (that have no need for accessors)
    bool                                   m_bDamageProof;
    std::uint32_t                                   m_uiDamageInfoSendPhase;
