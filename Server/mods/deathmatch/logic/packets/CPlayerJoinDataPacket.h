...
public:
    ...
    void SetNetVersion(std::uint16_t usNetVersion) noexcept { m_usNetVersion = usNetVersion; }
    void SetGameVersion(std::uint8_t ucGameVersion) noexcept { m_ucGameVersion = ucGameVersion; }
    ...
    void        SetNick(const char* szNick) noexcept { m_strNick.AssignLeft(szNick, MAX_PLAYER_NICK_LENGTH); }
