...
public:
    ...
    void      GetDescendantsByTypeSlow(std::vector<CElement*>& outResult, std::uint32_t uiTypeHash);
    template <class T>
    ...
    static std::uint32_t        GetTypeHashFromString(const SString& strTypeName);
...
protected:
    ...
    std::uint32_t                    m_uiChildrenListSnapshotRevision;
