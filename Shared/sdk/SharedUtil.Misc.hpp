...
SString SharedUtil::UnescapeString(const SString& strText, char cSpecialChar)
{
    ...
    for (auto i = 0; i < strText.length(); i++)
    {
        ...
        {
            auto c0 = FromHexChar(strText[++i]);
            auto c1 = FromHexChar(strText[++i]);
            c = (c0 << 4) | c1;
        }
        strResult += c;
    }
...
DWORD SharedUtil::GetMainThreadId()
{
    ...
    if (hThreadSnap != INVALID_HANDLE_VALUE)
    {
        ...
        for (BOOL bOK = Thread32First(hThreadSnap, &th32);; bOK = Thread32Next(hThreadSnap, &th32))
        {
...
//
bool SharedUtil::IsMainThread() noexcept
{
#ifdef _WIN32
...
int SharedUtil::GetBuildAge() noexcept
{
    tm when;
...
int SharedUtil::GetDaysUntilExpire() noexcept
{
    tm when;
    memset(&when, 0, sizeof(when));
    when.tm_year = BUILD_YEAR - 1900;
...
{
    const std::uint8_t* pCharData = (const std::uint8_t*)pData;
...
namespace SharedUtil
{
    ...
    bool CArgMap::Get(const SString& strCmd, std::vector<SString>& outList) const
    {
        ...
        for (std::uint32_t i = 0; i < newItems.size(); i++)
    ...
    //
    ///////////////////////////////////////////////////////////////////////////
    void GetThreadCPUTimes(std::uint64_t& outUserTime, std::uint64_t& outKernelTime) noexcept
    {
        outUserTime = 0;
        outKernelTime = 0;
#ifdef _WIN32
...
MTAEXPORT void GetLibMtaVersion(char* pBuffer, std::uint32_t uiMaxSize)
{
    SString strVersion("%d.%d.%d-%d.%05d.%d",
        ...
        MTASA_VERSION_TYPE, MTASA_VERSION_BUILD
#else
        0, 0, 0, 0, 0
#endif
        , 0);
